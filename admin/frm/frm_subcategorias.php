<?php
	...
	if($acao !=""){
		...
		$dados = new DadosSubCategoria();
		$dados->setId($id);
		$dados->mostrarSubcategoria();
...
<div id="formulario">
	<h2> Registo de Subcategorias </h2>
	<form action="./op/op_subcategorias.php" method="post" enctype="multipart/form-data">
		<label>
			<span class="titulo">Título da Subcategoria </span>
			<input type="text" name="txt_titulo" id="txt_titulo" value="<?php echo @$txt_titulo ?>">
		</label>
		
		<div class="dois-campos">
			<label>
				<span class="titulo">Ordem da Subcategoria </span>
				<input type="text" name="txt_ordem" id="txt_ordem" value="<?php echo @$txt_ordem ?>">
			</label>
			<label>
				<span class="titulo">Activo da Subcategoria </span>
