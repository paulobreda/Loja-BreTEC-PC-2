<?php
	...
	if($acao=="Inserir"){
		$cad ->setCampos("titulo_banner, alt, url_banner,activo_banner,imagem_banner");
	...
	if($acao=="Alterar"){
		$cad ->setCampos("	titulo_banner 	='$txt_titulo', 
							alt				='$txt_alt', 
							url_banner 		='$txt_url',
							activo_banner 	='$txt_ativo',
							imagem_banner 	='$txt_nomeimagem' ");
		$cad->setValorNaTabela("id_banner");
		$cad->setValorPesquisa("$id");
		$cad->alterar();
		
		echo "<script type='text/javascript'> location.href='../principal.php?link=10' </script> ";
	}
